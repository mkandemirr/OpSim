...
EJ426Builder::EJ426Builder(G4bool enableOpticalProperty):
VMaterialBuilder(enableOpticalProperty) 
{
  if(IsOpticalPropertyEnabled())
  {
    pMatPropTable = new MaterialPropertiesTable("EJ-426"); 
    
    G4UImanager* uImanager = G4UImanager::GetUIpointer();
    uImanager->ApplyCommand("/control/execute EljenScintillators/macros/ej-426.mac");  
  }
...
{ 
  ...
  G4Material *scin_mat    = new G4Material("EJ-426", density=2.42*g/cm3, nComponents =8);
