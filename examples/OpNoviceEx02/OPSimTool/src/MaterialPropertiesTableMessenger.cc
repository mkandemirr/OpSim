...
//for G4UnitDefinition
#include "G4UnitsTable.hh"
 

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

MaterialPropertiesTableMessenger::MaterialPropertiesTableMessenger(MaterialPropertiesTable* mpt )
:G4UImessenger(), 
fMatPropTable{mpt}, 
fDirectory1{nullptr}, 
fDirectory2{nullptr},
...
{  
  ...
  if(!fDirectory1)
  {
    ...
    fDirectory1->SetGuidance("Commands for objects of MaterialPropertiesTable class.");  
  }
...
void MaterialPropertiesTableMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if( command==fAddPropertyCmd )
  {
    ...
    if(createNewKey.empty())
    {
      ...
      fMatPropTable->AddProperty(key,path,energyUnit,valueUnit,StoB(createNewKey));
      
    }else if(propertyValue.empty()){
      fMatPropTable->AddProperty(key,path,energyUnit,valueUnit,StoB(createNewKey),StoB(spline));
    }else{
      fMatPropTable->AddProperty(key,path,energyUnit,valueUnit,StoB(createNewKey),StoB(spline),StoD(propertyValue));
    }
    ...
    if( unit.empty() )
    {
      ...
      G4double dimensionedDouble = fAddConstPropertyCmd->ConvertToDimensionedDouble(r);
